$(function () {
    layui.use(['jquery', 'table', 'laydate'], function () {
        ...
        $("#searchBtn").click(function () {
            ...
            tableInit(startTime, endTime, contractNum);

        });

        tableInit(null, null, null);

        function tableInit(startTime, endTime, contractNum) {
            table.render({
                elem: '#table111'//表格的id值
                , id: 'idTest'//当需要获取选中数据时所需的id即lay-data="{id: 'idTest'}"中的idTest，注意这里的id不用写#
                //, height: 350//表格的高度
                , url: '../../../contract/findAllByCondition' //数据接口
                , method: 'post'//请求方式
                ,toolbar:'#toolbarDemo'
                , limit: 10//每页显示的条数
                ...
                , cols: [[ //表头
                    ...
                    , {
                        field: 'buyer', title: '甲方用户名', templet: function (d) {
                            return d.buyer.username;
                        }
                    }
                    , {
                        field: 'seller', title: '乙方用户名', templet: function (d) {
                            return d.seller.username;
                        }
                    }
                    , {
                        field: 'sellParkingBean', title: '车辆位置', templet: function (d) {
                            return d.sellParkingBean.parkingBean.address;
                        }
                    }
