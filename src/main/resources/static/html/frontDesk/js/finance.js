$(function () {
    ...
    $.ajax({
        type: "post",
        url: "/personalBill/PersonalCount",
        dataType: "json",
...
function tableInit(startTime, endTime) {
    ...
    $("#tb").bootstrapTable({
        url: "/personalBill/showPersonalPageByCondition",//请求的url地址
        method: "post",//请求方式
        pageSize: 10,//每页显示的数量
        pageList: [3,5,10, 20],//提供选择每页显示多少条
        ...
        responseHandler: function (res) {
            return {
                "total": res.data.personalBill.total,//当采用服务端分时必须给total赋值即从服务端得到总条数，表格将自动根据总条数进行分页
                "rows": res.data.personalBill.records //每行显示的数据集合，注意必须是（total和rows不能是其他的字段，否则将不能显示数据或分页）
        ...
        columns: [{
            ...
            {field: "dealDate", title: "交易日期", sortable: true},//表头，其中field值是返回JSON格式中对应的属性
            {field: "dealTime", title: "交易时间", sortable: true},
